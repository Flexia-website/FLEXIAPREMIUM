# backend/app.py - COMPLETE PRODUCTION VERSION v17.0
...
def register():
    ...
    try:
        ...
        game_stats = json.dumps({"snake": {"high_score": 0, "total_score": 0}, "coin_flip": {"wins": 0, "losses": 0, "current_streak": 0}, "plinko": {"total_wins": 0, "total_bets": 0, "highest_win": 0}})
        ...
        ''', (username, generate_password_hash(password), 0.00, user_referral_code, referral_code or None, is_admin_value, datetime.utcnow().isoformat(), datetime.utcnow().isoformat(), game_stats, contact or "", "", "light", admin_pw_changed, None, withdrawal_restricted, 0.00, 0, 0, datetime.utcnow().isoformat(), datetime.utcnow().isoformat(), '[]', 0, datetime.utcnow().isoformat()))
        if is_postgres:
        ...
        response = jsonify({"success": True, "message": "Registration successful", "user": {"id": new_id, "username": username, "referral_code": user_referral_code, "balance": admin_bonus}})
        secure_cookie = (os.getenv('ENV') == 'production')
...
def login():
    ...
    try:
        ...
        conn.commit()
        resp = jsonify({"success": True, "user": {"id": user['id'], "username": user['username'], "balance": float(user['balance']) if user['balance'] else 0.00, "referral_code": user.get('referral_code', ''), "is_admin": bool(user.get('is_admin', False)), "admin_password_changed": bool(user.get('admin_password_changed', False)), "profile_picture": user.get('profile_picture', ''), "ui_theme": user.get('ui_theme', 'light'), "transactions": []}})
        token = create_session_token(user['id'])
        secure_cookie = (os.getenv('ENV') == 'production')
        resp.set_cookie('session_token', token, httponly=True, secure=secure_cookie, samesite='Lax', max_age=86400)
...
def validate_coupon():
    ...
    try:
        ...
        row = cursor.fetchone()
        if not row:
            return jsonify({"success": False, "message": "Invalid coupon code. Please check and try again."}), 404
        status = row[0]
        metadata = json.loads(row[1]) if row[1] else {}
        if status != 'AVAILABLE':
            if status == 'USED':
                return jsonify({"success": False, "message": "This coupon code has already been used."}), 403
            else:
                return jsonify({"success": False, "message": f"This coupon code is {status}. Please contact support."}), 403
        if metadata.get('expires_at'):
            try:
                expiry = datetime.fromisoformat(metadata['expires_at'])
                if datetime.utcnow() > expiry:
                    return jsonify({"success": False, "message": "This coupon code has expired."}), 403
            except:
                pass
        return jsonify({"success": True, "message": "Coupon code is valid", "coupon_code": coupon_code, "generated_by": metadata.get('generated_by', 'unknown'), "amount_paid": metadata.get('amount_paid', 0)})
    except Exception as e:
        app.logger.error(f"Coupon validation error: {e}")
        return jsonify({"success": False, "message": "Failed to validate coupon. Please try again."}), 500
    finally:
        return_db_connection(conn)
...
def get_user_profile():
    ...
    try:
        ...
        return jsonify({"success": True, "user": {"id": fresh_user['id'], "username": fresh_user['username'], "balance": float(fresh_user['balance']) if fresh_user['balance'] else 0.00, "referral_code": fresh_user.get('referral_code', ''), "is_admin": bool(fresh_user.get('is_admin', False)), "created_at": fresh_user.get('created_at', ''), "game_stats": json.loads(fresh_user.get('game_stats', '{}')), "transactions": transactions, "withdrawal_pin": bool(fresh_user.get('withdrawal_pin')), "profile_picture": fresh_user.get('profile_picture', ''), "ui_theme": fresh_user.get('ui_theme', 'light'), "claimed_achievements": json.loads(fresh_user.get('claimed_achievements', '[]'))}, "referrals": {"count": referrals, "unclaimed_bonus": unclaimed}})
    except Exception as e:
    ...
    try:
        ...
        if row and row[0]:
            try:
                ...
                pass
        return jsonify({"success": True, "can_withdraw": can_withdraw, "today": today, "global_withdrawal_days": global_days, "custom_withdrawal_days": custom_days if custom_days else [], "message": f"You {'CAN' if can_withdraw else 'CANNOT'} withdraw today. Withdrawal days: {', '.join(map(str, sorted(custom_days if custom_days else global_days)))}"})
    except Exception as e:
...
def check_game_access_simple():
    ...
    try:
        ...
        return jsonify({"success": True, "can_play": can_play, "played_today": plays_today, "max_plays": max_plays, "remaining_plays": max(0, max_plays - plays_today)})
    except Exception as e:
...
def check_game_access(game_type):
    ...
    try:
        ...
        return jsonify({"success": True, "can_play": can_play, "played_today": plays_today, "max_plays": max_plays, "remaining": remaining, "game_type": game_type, "game_name": get_game_friendly_name(game_type), "message": f"You have {remaining} {game_type} plays remaining today" if can_play else f"Daily limit reached: {plays_today}/{max_plays} plays"})
    except Exception as e:
        app.logger.error(f"Game access check error: {e}")
        return jsonify({"success": False, "message": "Failed to check game access"}), 500
...
def check_game_limit():
    ...
    try:
        plays_today = get_game_plays_today(user['id'], game_type)
        max_plays = CONFIG.GAME_DAILY_LIMITS.get(game_type, 5)
        return jsonify({"success": True, "can_play": plays_today < max_plays, "played_today": plays_today, "max_per_day": max_plays, "remaining": max(0, max_plays - plays_today)})
    except Exception as e:
        app.logger.error(f"Game limit check error: {e}")
...
def check_game_limit_with_logout_endpoint(game_type):
    ...
    try:
        ...
        if result.get("can_play", False):
            return jsonify({"success": True, "can_play": True, "played_today": result.get("played_today", 0), "max_plays": result.get("max_plays", 5), "remaining": result.get("remaining", 0), "game_type": game_type, "game_name": get_game_friendly_name(game_type)})
        else:
            return jsonify({"success": False, "can_play": False, "reason": result.get("reason", "Daily limit reached"), "action_required": result.get("action_required", "logout"), "game_type": game_type, "played_today": result.get("played_today", 0), "max_plays": result.get("max_plays", 5), "reset_time": result.get("reset_time", "00:00 UTC"), "force_logout": True, "message": f"Daily limit reached! You've played {result.get('played_today', 0)}/{result.get('max_plays', 5)} times today. Please come back tomorrow after 00:00 UTC."}), 403
    except Exception as e:
        app.logger.error(f"Game limit check error: {e}")
...
def force_logout_from_game(game_type):
    ...
    try:
        ...
        cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user['id'], 'FORCE_LOGOUT', 0, 'COMPLETED', json.dumps({"gt":game_type,"r":"limit"}), datetime.utcnow().isoformat()))
        conn.commit()
    except Exception as e:
        ...
        return_db_connection(conn)
    resp = jsonify({"success": True, "force_logout": True, "reason": f"Daily {game_type} limit reached", "redirect": "/?reason=daily_limit_reached", "message": "You have reached your daily limit. Please come back tomorrow!"})
    resp.set_cookie('session_token', '', expires=0)
    return resp
...
def report_snake_enhanced():
    ...
    try:
        ...
        if not limit_check.get("can_play", False):
            return jsonify({"success": False, "message": f"Daily snake game limit reached! {limit_check.get('reason', '')}", "force_logout": True, "redirect": True, "details": limit_check}), 403
        ...
        try:
            ...
            if earned_today >= SNAKE_DAILY_CAP:
                return_db_connection(conn)
                return jsonify({"success": False, "message": f"Daily snake earnings cap reached (₦{SNAKE_DAILY_CAP}). Come back tomorrow!", "daily_earned": earned_today, "daily_cap": SNAKE_DAILY_CAP}), 400
            ...
            cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user['id'], 'SNAKE_REWARD', reward, 'COMPLETED', json.dumps({"g":"snake","a":apples,"ga":golden_apples}), datetime.utcnow().isoformat()))
            conn.commit()
            app.logger.info(f"Snake reward granted to {user['username']}: ₦{reward}")
            remaining_cap = max(0, SNAKE_DAILY_CAP - (earned_today + reward))
            return jsonify({"success": True, "reward": reward, "new_balance": new_balance, "apples": apples, "transaction_id": tx_id, "daily_earned": earned_today + reward, "daily_cap": SNAKE_DAILY_CAP, "remaining_today": remaining_cap, "message": f"Success! Claimed ₦{reward} for {apples} apples. Daily cap: ₦{int(earned_today+reward)}/₦{SNAKE_DAILY_CAP}"})
        except Exception as e:
...
def report_coinflip_enhanced():
    ...
    try:
        ...
        if not limit_check.get("can_play", False):
            return jsonify({"success": False, "message": f"Daily coin flip limit reached! {limit_check.get('reason', '')}", "force_logout": True, "redirect": True, "details": limit_check}), 403
        ...
        try:
            ...
            if won:
                ...
                if won_today >= COINFLIP_WIN_CAP:
                    return_db_connection(conn)
                    return jsonify({"success": False, "message": f"Daily coinflip win cap reached (₦{COINFLIP_WIN_CAP}). Come back tomorrow!", "daily_won": won_today, "daily_cap": COINFLIP_WIN_CAP}), 400
            ...
            cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user['id'], tx_type, net_change, 'COMPLETED', json.dumps({"g": "coin", "b": bet, "w": int(won)}), datetime.utcnow().isoformat()))
            conn.commit()
            ...
            return jsonify({"success": True, "payout": payout if won else 0, "net_change": net_change, "new_balance": new_balance, "won": won, "daily_won": daily_won_after, "daily_cap": COINFLIP_WIN_CAP, "house_edge_note": "Wins pay 1.8x your bet", "message": f"You {'won' if won else 'lost'}! {'+' if won else '-'}₦{abs(net_change):.2f}"})
        except Exception as e:
...
def report_plinko_enhanced():
    ...
    try:
        ...
        if not limit_check.get("can_play", False):
            return jsonify({"success": False, "message": f"Daily plinko limit reached! {limit_check.get('reason', '')}", "force_logout": True, "redirect": True, "details": limit_check}), 403
        ...
        try:
            ...
            if net_change > 0:
                ...
                if plinko_won_today >= PLINKO_WIN_CAP:
                    return_db_connection(conn)
                    return jsonify({"success": False, "message": f"Daily plinko win cap reached (₦{PLINKO_WIN_CAP}). Come back tomorrow!", "daily_won": plinko_won_today, "daily_cap": PLINKO_WIN_CAP}), 400
            ...
            cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user['id'], tx_type, net_change, 'COMPLETED', json.dumps({"g": "plinko", "b": bet, "x": multiplier}), datetime.utcnow().isoformat()))
            conn.commit()
            ...
            return jsonify({"success": True, "win_amount": win_amount, "net_change": net_change, "new_balance": new_balance, "multiplier": multiplier, "daily_won": daily_plinko_after, "daily_cap": PLINKO_WIN_CAP, "message": f"Plinko result: ×{multiplier} = {'+' if net_change > 0 else ''}₦{net_change:.2f}"})
        except Exception as e:
...
def execute_spin():
    ...
    try:
        ...
        cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user['id'], 'SPIN_REWARD', reward, 'COMPLETED', json.dumps({"g": "spin"}), datetime.utcnow().isoformat()))
        conn.commit()
        ...
        return jsonify({"success": True, "reward": reward, "prize_index": prize_index, "new_balance": new_balance, "is_bonus_spin": is_bonus_spin, "message": msg})
    except Exception as e:
...
def get_tiktok_daily_task():
    ...
    try:
        ...
        if task_row:
            task = {'tiktok_link': task_row[0], 'reward_amount': float(task_row[1]) if task_row[1] else CONFIG.TIKTOK_REWARD}
...
def follow_tiktok_daily_enhanced():
    ...
    try:
        ...
        if not limit_check.get("can_play", False):
            return jsonify({"success": False, "message": f"Daily TikTok limit reached! {limit_check.get('reason', '')}", "force_logout": True, "redirect": True, "details": limit_check}), 403
        new_balance = update_user_balance(user['id'], reward)
        if new_balance is None:
            return jsonify({"success": False, "message": "Failed to update balance"}), 500
        tx_id = f"TIKTOK-{secrets.token_hex(8)}"
        cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, timestamp) VALUES (%s, %s, %s, %s, %s, %s)', (tx_id, user['id'], 'TIKTOK_DAILY', reward, 'COMPLETED', datetime.utcnow().isoformat()))
        conn.commit()
        return_db_connection(conn)
        ...
        return jsonify({"success": True, "reward": reward, "new_balance": new_balance, "task_number": task_number, "next_reward": next_reward, "message": f"Task {task_number}/3 complete! Earned ₦{reward}" + (f" | Next task: ₦{next_reward}" if next_reward else " | All tasks done today!")})
    except Exception as e:
...
def claim_daily_login_bonus():
    ...
    try:
        ...
        if last_login_date == today:
            return jsonify({"success": False, "message": "Already claimed today's login bonus. Come back tomorrow!", "streak": current_streak}), 400
        ...
        cursor.execute(f"INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES ({ph}, {ph}, {ph}, {ph}, {ph}, {ph}, {ph})", (tx_id, user['id'], 'LOGIN_BONUS', reward, 'COMPLETED', json.dumps({"streak": new_streak}), datetime.utcnow().isoformat()))
        conn.commit()
        next_milestone = None
        if new_streak < 7:
        ...
        return jsonify({"success": True, "reward": reward, "new_balance": new_balance, "streak": new_streak, "milestone": milestone, "next_milestone": next_milestone, "message": f"{'🎉 ' + milestone + ' ' if milestone else ''}Daily bonus: ₦{reward} | Streak: {new_streak} days"})
    except Exception as e:
...
def get_login_bonus_status():
    ...
    try:
        ...
        if (streak + 1) >= 30:
            ...
            next_reward = 100
        return jsonify({"success": True, "streak": streak, "already_claimed": already_claimed, "next_reward": next_reward, "last_claimed": last_date})
    except Exception as e:
...
def claim_referral_bonus():
    ...
    try:
        ...
        cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user['id'], 'REFERRAL_BONUS', unclaimed, 'COMPLETED', json.dumps({"refs": referrals}), datetime.utcnow().isoformat()))
        conn.commit()
        return_db_connection(conn)
        app.logger.info(f"Referral bonus claimed by {user['username']}: {unclaimed}")
        return jsonify({"success": True, "claimed": unclaimed, "new_balance": new_balance, "message": f"Success! Claimed ₦{unclaimed} referral bonus"})
    except Exception as e:
...
def get_achievements():
    ...
    try:
        ...
        return jsonify({"success": True, "stats": {"total": total_achievements, "unlocked": unlocked_achievements, "unlocked_not_claimed": unlocked_not_claimed, "points": total_points}, "achievements": achievements_data, "current_balance": fresh_balance, "has_unclaimed_rewards": unlocked_not_claimed > 0})
    except Exception as e:
...
def claim_achievement_rewards():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Achievement rewards processed successfully", "new_balance": new_balance})
    except Exception as e:
...
def withdraw():
    ...
    try:
        ...
        cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user['id'], 'WITHDRAWAL', amount, 'PENDING', json.dumps({'bc':bank_code,'an':account_number,'nm':account_name}), datetime.utcnow().isoformat()))
        conn.commit()
        app.logger.info(f"Withdrawal requested by {user['username']}: {amount} to {bank_code}:{account_number}")
        return jsonify({"success": True, "message": "Withdrawal submitted successfully", "transaction_id": tx_id, "new_balance": new_balance})
    except Exception as e:
...
def initialize_paystack_payment():
    ...
    try:
        ...
        payload = {'email': email, 'amount': amount_kobo, 'reference': reference, 'callback_url': PAYSTACK_CALLBACK_URL, 'metadata': json.dumps({'user_id': user['id'], 'username': user['username'], 'coupon_amount': amount, 'type': 'coupon_purchase', 'email': email})}
        response = requests.post('https://api.paystack.co/transaction/initialize', headers=headers, json=payload)
        data = response.json()
        if data.get('status'):
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (f"PAY-{secrets.token_hex(8)}", user['id'], 'PAYSTACK_INIT', amount, 'PENDING', json.dumps({'reference': reference, 'email': email, 'amount': amount, 'authorization_url': data['data']['authorization_url'], 'bank_transfer': data['data'].get('bank_transfer'), 'expires_at': (datetime.utcnow() + timedelta(minutes=60)).isoformat()}), datetime.utcnow().isoformat()))
            conn.commit()
            return_db_connection(conn)
            return jsonify({"success": True, "authorization_url": data['data']['authorization_url'], "reference": reference, "bank_transfer_details": data['data'].get('bank_transfer'), "expires_at": (datetime.utcnow() + timedelta(minutes=60)).isoformat(), "message": "Payment initialized successfully"})
        else:
...
def paystack_callback():
    try:
        ...
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('UPDATE transactions SET status = %s, details = %s WHERE type = \'PAYSTACK_INIT\' AND details LIKE %s', ('COMPLETED', json.dumps({'reference': reference, 'coupon_code': coupon_code if coupon_code else None, 'email': email, 'amount': amount, 'payment_method': payment_method, 'coupon_sent': email_sent, 'coupon_generated': coupon_code is not None, 'minimum_amount_required': MIN_COUPON_AMOUNT, 'payment_accepted': True, 'paystack_data': transaction}), f'%{reference}%'))
        conn.commit()
        return_db_connection(conn)
...
def get_paystack_status(reference):
    ...
    try:
        ...
        return jsonify({"success": True, "status": status, "coupon_code": details.get('coupon_code'), "email": details.get('email'), "amount": details.get('amount'), "payment_method": details.get('payment_method', 'bank_transfer'), "coupon_generated": details.get('coupon_generated', False), "coupon_sent": details.get('coupon_sent', False), "payment_accepted": details.get('payment_accepted', True), "minimum_amount_required": details.get('minimum_amount_required', 8000), "message": "Coupon generated and sent!" if details.get('coupon_generated') else f"Payment accepted. Minimum ₦{details.get('minimum_amount_required', 8000)} required for coupon."})
    except Exception as e:
...
def admin_get_users():
    ...
    try:
        ...
        users = []
        for row in cursor.fetchall():
            user = {'id': row[0], 'username': row[1], 'balance': float(row[2]) if row[2] else 0.00, 'referral_code': row[3], 'withdrawal_restricted': bool(row[4]), 'withdrawal_limit': float(row[5]) if row[5] else 0.00, 'created_at': row[6], 'last_login': row[7], 'is_admin': bool(row[8])}
            users.append(user)
...
def admin_adjust_user_balance(user_id):
    ...
    try:
        ...
        cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx_id, user_id, 'ADMIN_ADJUSTMENT', amount, 'COMPLETED', json.dumps({"note": note[:80] if note else ""}), datetime.utcnow().isoformat()))
        conn.commit()
        app.logger.info(f"Admin adjusted balance for user {user_id}: {amount} (note: {note})")
        return jsonify({"success": True, "message": "Balance adjusted", "new_balance": new_balance, "adjustment": amount})
    except Exception as e:
...
def admin_update_settings():
    ...
    try:
        ...
        ph = '%s' if os.environ.get('DATABASE_URL') else '?'
        if min_withdrawal is not None:
            cursor.execute(f'UPDATE admin_settings SET whatsapp_link = {ph}, telegram_link = {ph}, facebook_link = {ph}, global_withdrawal_days = {ph}, min_withdrawal = {ph}', (whatsapp_link, telegram_link, facebook_link, json.dumps(global_withdrawal_days), min_withdrawal))
        else:
            cursor.execute(f'UPDATE admin_settings SET whatsapp_link = {ph}, telegram_link = {ph}, facebook_link = {ph}, global_withdrawal_days = {ph}', (whatsapp_link, telegram_link, facebook_link, json.dumps(global_withdrawal_days)))
        conn.commit()
...
def admin_get_stats():
    ...
    try:
        ...
        return jsonify({"success": True, "stats": {"users": {"total": total_users, "today": today_users}, "balance": {"total": float(total_balance)}, "transactions": {"total": total_transactions, "today_withdrawals": today_withdrawals, "total_withdrawn": float(total_withdrawn)}, "coupons": {"total": total_coupons, "available": available_coupons}, "games": {"today": today_games}}})
    except Exception as e:
...
def admin_withdrawal_status_report():
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            users.append({'id': user_id, 'username': username, 'balance': balance, 'withdrawal_restricted': withdrawal_restricted, 'custom_withdrawal_days': json.loads(custom_days_str) if custom_days_str else [], 'withdrawal_limit': withdrawal_limit, 'has_withdrawal_pin': has_withdrawal_pin, 'can_withdraw_today': can_withdraw_today})
        return jsonify({"success": True, "today": datetime.utcnow().strftime("%d %B %Y"), "total_users": total_users, "users_withdrawal_today": users_withdrawal_today, "users_restricted": users_restricted, "global_withdrawal_days": global_days, "users": users})
    except Exception as e:
...
def admin_get_whatsapp_numbers():
    ...
    try:
        ...
        numbers = []
        for row in cursor.fetchall():
            num = {'id': row[0], 'number': row[1], 'label': row[2], 'is_active': bool(row[3]), 'created_at': row[4]}
            numbers.append(num)
        return jsonify({"success": True, "numbers": numbers})
    except Exception as e:
...
def admin_add_whatsapp_number():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('INSERT INTO whatsapp_numbers (number, label, is_active, created_at) VALUES (%s, %s, %s, %s)', (number, label, True, datetime.utcnow().isoformat()))
        conn.commit()
...
def admin_set_tiktok_daily():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('INSERT INTO tiktok_daily (date, tiktok_link, reward_amount) VALUES (%s, %s, %s) ON CONFLICT (date) DO UPDATE SET tiktok_link = EXCLUDED.tiktok_link, reward_amount = EXCLUDED.reward_amount', (today, tiktok_link, CONFIG.TIKTOK_REWARD))
        conn.commit()
...
def admin_get_tiktok_daily():
    ...
    try:
        ...
        row = cursor.fetchone()
        if row:
            task = {'date': row[0], 'tiktok_link': row[1], 'reward_amount': float(row[2]) if row[2] else CONFIG.TIKTOK_REWARD}
...
def admin_get_tiktok_history():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('SELECT date, tiktok_link, reward_amount FROM tiktok_daily WHERE date >= %s ORDER BY date DESC LIMIT 7', ((datetime.utcnow().date() - timedelta(days=7)).isoformat(),))
        history = []
        for row in cursor.fetchall():
            history.append({'date': row[0], 'tiktok_link': row[1], 'reward_amount': float(row[2]) if row[2] else CONFIG.TIKTOK_REWARD})
...
def admin_clear_database():
    ...
    try:
        ...
        cursor.execute('UPDATE users SET balance = 500000.00, claimed_bonuses = 0, points = 0, game_stats = %s, last_game_timestamp = %s, last_achievement_check = %s, claimed_achievements = \'[]\' WHERE id = %s', (json.dumps({"snake": {"high_score": 1200, "total_score": 5000}, "coin_flip": {"wins": 25, "losses": 18, "current_streak": 3}, "plinko": {"total_wins": 15, "total_bets": 25000, "highest_win": 5000}}), datetime.utcnow().isoformat(), datetime.utcnow().isoformat(), admin_id))
        ...
        backup_file = backup_database()
        return jsonify({"success": True, "message": "Database cleared successfully!", "stats": {"users_deleted": users_deleted, "transactions_deleted": transactions_deleted, "game_plays_deleted": game_plays_deleted, "coupons_reset": coupons_reset, "admin_kept": admin_username, "backup_created": backup_file if backup_file else "No backup"}, "warning": "ALL USER DATA HAS BEEN PERMANENTLY DELETED. This action cannot be undone."})
    except Exception as e:
...
def admin_preview_database_clear():
    ...
    try:
        ...
        admin_row = cursor.fetchone()
        return jsonify({"success": True, "preview": {"users_to_delete": users_count, "transactions_to_delete": transactions_count, "game_plays_to_delete": game_plays_count, "coupons_to_reset": coupons_count, "used_coupons_to_reset": used_coupons_count, "admin_to_keep": {"id": admin_row[0], "username": admin_row[1], "balance": float(admin_row[2]) if admin_row[2] else 0}, "warning": "This action is irreversible. All user data will be permanently deleted.", "sudo_confirmation_required": "DELETE_ALL_DATA_AND_USERS_KEEP_ADMIN"}})
    except Exception as e:
...
def admin_export_all_data():
    try:
        ...
        data['_metadata'] = {'export_date': datetime.utcnow().isoformat(), 'version': 'FLEXIA_BACKUP_v1.0', 'total_records': {'users': len(users), 'transactions': len(transactions), 'game_plays': len(game_plays), 'coupons': len(coupons)}}
        return_db_connection(conn)
        ...
        try:
            ...
            if 'transactions' in data:
                for tx in data['transactions']:
                    try:
                        cursor.execute('INSERT INTO transactions (id, user_id, type, amount, status, details, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)', (tx.get('id'), tx.get('user_id'), tx.get('type'), tx.get('amount'), tx.get('status'), tx.get('details'), tx.get('timestamp')))
            ...
            if 'game_plays' in data:
                for play in data['game_plays']:
                    try:
                        cursor.execute('INSERT INTO game_plays (id, user_id, game_type, play_date, created_at) VALUES (%s, %s, %s, %s, %s)', (play.get('id'), play.get('user_id'), play.get('game_type'), play.get('play_date'), play.get('created_at')))
            ...
            if 'coupons' in data:
                for coupon in data['coupons']:
                    try:
                        cursor.execute('INSERT INTO coupons (code, status) VALUES (%s, %s)', (coupon.get('code'), coupon.get('status', 'AVAILABLE')))
            ...
            if 'whatsapp_numbers' in data:
                for num in data['whatsapp_numbers']:
                    try:
                        cursor.execute('INSERT INTO whatsapp_numbers (id, number, label, is_active, created_at) VALUES (%s, %s, %s, %s, %s)', (num.get('id'), num.get('number'), num.get('label'), num.get('is_active'), num.get('created_at')))
            ...
            if 'tiktok_daily' in data:
                for tiktok in data['tiktok_daily']:
                    try:
                        cursor.execute('INSERT INTO tiktok_daily (id, date, tiktok_link, reward_amount, created_at) VALUES (%s, %s, %s, %s, %s)', (tiktok.get('id'), tiktok.get('date'), tiktok.get('tiktok_link'), tiktok.get('reward_amount'), tiktok.get('created_at')))
            ...
            if 'admin_settings' in data:
                for setting in data['admin_settings']:
                    try:
                        cursor.execute('INSERT INTO admin_settings (id, whatsapp_link, telegram_link, facebook_link, global_withdrawal_days) VALUES (%s, %s, %s, %s, %s)', (setting.get('id'), setting.get('whatsapp_link'), setting.get('telegram_link'), setting.get('facebook_link'), setting.get('global_withdrawal_days')))
            ...
            return jsonify({"success": True, "message": "Database imported successfully!", "imported_counts": imported_counts, "metadata": data.get('_metadata', {})})
        except Exception as e:
            conn.rollback()
    ...
    try:
        ...
        for filename in sorted(os.listdir('backups'), reverse=True):
            ...
            if os.path.isfile(filepath):
                ...
                backups.append({"filename": filename, "size": stat.st_size, "created": datetime.fromtimestamp(stat.st_mtime).isoformat(), "size_human": f"{stat.st_size / 1024 / 1024:.2f} MB"})
...
def api_health():
    try:
        ...
        return_db_connection(conn)
        health_data = {"status": "online", "service": "FLEXIA API", "timestamp": datetime.utcnow().isoformat(), "uptime": get_uptime(), "database": db_status, "version": "17.0", "stats": {"total_users": user_count, "pending_withdrawals": pending_withdrawals}, "environment": os.getenv('ENV', 'development'), "connection_pool": "active" if db_pool else "inactive"}
        app.logger.info(f"Health check passed: {health_data}")
        return jsonify(health_data), 200
    except Exception as e:
        app.logger.error(f"Health check failed: {str(e)}")
        return jsonify({"status": "degraded", "service": "FLEXIA API", "timestamp": datetime.utcnow().isoformat(), "database": f"error: {str(e)}", "uptime": get_uptime(), "version": "17.0"}), 503
...
if __name__ == '__main__':
    ...
    app.logger.info(f"Starting Flexia Platform v17.0 on port {port} (debug: {debug})")
